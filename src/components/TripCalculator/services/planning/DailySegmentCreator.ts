...
export class DailySegmentCreator {
  ...
  static createBalancedDailySegments(
    ...
    // FIXED: Filter to only use destination cities for overnight stops with explicit typing
  ...
  private static selectDestinationCityOvernightStops(
    ...
    // Filter out start and end stops - explicitly type the result
    const availableStops: TripStop[] = destinationCities.filter((stop) => {
    ...
    const sortedStops: TripStop[] = availableStops.sort((a, b) => {
    ...
    const finalStops: TripStop[] = overnightStops.sort((a, b) => {
      const distA = DistanceCalculationService.calculateDistance(
        startStop.latitude, startStop.longitude, a.latitude, a.longitude
      );
      const distB = DistanceCalculationService.calculateDistance(
  ...
  private static findStrictAttractionsForSegment(
    ...
    // Find stops along the route - EXCLUDE destination cities for attractions with explicit typing
    const attractions: TripStop[] = allStops.filter((stop) => {
    ...
    const sortedAttractions: TripStop[] = attractions.sort((a, b) => {
