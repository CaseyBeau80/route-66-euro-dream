...
export class DestinationFocusedPlanningService {
  static async planDestinationFocusedTrip(
    ...
    console.log(`🎯 FIXED DESTINATION-FOCUSED PLANNING: ${startCityName} to ${endCityName} in ${tripDays} days`);
    ...
    const majorDestinations = allStops.filter(stop => stop.is_major_stop);

    // Enforce Route 66 sequence
    const sequenceResult = Route66SequenceEnforcer.enforceSequenceDirection(
      startStop, endStop, majorDestinations
    );
    ...
    // Select major intermediate destinations
