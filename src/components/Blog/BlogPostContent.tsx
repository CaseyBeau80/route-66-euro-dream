...
const MarkdownBlock: React.FC<{ content: string; isEventCard?: boolean; index?: number }> = ({ content, isEventCard, index = 0 }) => {
  ...
  const markdownComponents = {
    a: ({ href, children }: any) => {
      ...
      if (isInternal) {
        return <Link to={href} className={`${linkColor} font-medium hover:underline`}>{children}</Link>;
      }
      return (
        <a href={href} target="_blank" rel="noopener noreferrer" className={`${linkColor} font-medium hover:underline`}>
          {children}
        </a>
      );
    },
  ...
  return (
    <div className={isEventCard 
      ? `${isDark ? 'bg-[#1B2A4A] text-white [&_em]:text-[#F5F0E8] [&_span]:text-inherit' : 'bg-[#FAFAF7] border border-route66-sand/60'} rounded-lg shadow-sm p-5 md:p-6 my-6` 
      : ''
    }>
      {isEventCard && states.length > 0 && eventFields ? (
        <>
          ...
          <div className="flex items-center gap-3">
            ...
            {eventFields.before && (
              <div className={`font-playfair font-bold text-lg leading-snug ${isDark ? 'text-white' : 'text-route66-brown'}`}>
                <ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>
                  {eventFields.before.replace(/\s*\((IL|MO|KS|OK|TX|NM|AZ|CA)\)\s*$/g, '')}
                </ReactMarkdown>
              </div>
            )}
          </div>
