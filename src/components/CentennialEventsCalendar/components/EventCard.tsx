...
import { getSmartCountdownText, isEventSoon, formatSmartDateDisplay } from '../utils/eventCalendarHelpers';
      ...
      >
        ...
        <div className="bg-gradient-to-r from-slate-50 to-blue-50/30 p-3 border-b border-slate-200">
          <div className="flex items-center justify-between gap-2 mb-2">
            <Badge variant="secondary" className={`${stateInfo.color} text-white text-xs font-bold`}>
        ...
        <CardContent className="p-3 space-y-2">
          {event.guinnessAttempt && <GuinnessBadge size="sm" />}
          
          <div className="flex items-center gap-2 text-sm text-slate-700">
            <Calendar className="h-4 w-4 text-[#1B60A3] flex-shrink-0" />
            <span className="font-medium">{formatSmartDateDisplay(event)}</span>
          </div>
          ...
          <div className="flex items-start gap-2 text-sm text-slate-600">
            ...
            <div>
              ...
              {event.venue && <span className="block text-xs text-slate-500">{event.venue}</span>}
            </div>
          </div>
