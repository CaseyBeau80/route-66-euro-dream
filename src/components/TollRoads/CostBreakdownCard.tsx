    ...
    <Card className="border-4 border-route66-red bg-white shadow-2xl">
      <CardContent className="p-10">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-900 max-w-3xl mx-auto text-xl font-semibold">{estimatedCosts.description}</p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {estimatedCosts.details.map((detail, index) => (
            <div key={index} className="bg-route66-red p-6 rounded-xl shadow-lg border-3 border-route66-vintage-brown">
              <p className="text-base font-bold text-white text-center">{detail}</p>
            </div>
          ))}
        </div>
        
        <div className="mt-8 p-6 bg-route66-orange rounded-xl border-3 border-route66-red">
