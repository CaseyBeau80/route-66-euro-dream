  ...
  useEffect(() => {
    const fetchAllSlugs = async () => {
      const [attractions, hiddenGems, blogPosts, upcomingEvents, nativeHeritage] = await Promise.all([
        ...
        supabase.from('upcoming_events').select('event_id'),
      ...
      setXml(generateSitemapFile({
        ...
        eventIds: upcomingEvents.data?.map((r: any) => r.event_id) ?? [],
        nativeHeritageSlugs: nativeHeritage.data?.map((r: any) => r.slug) ?? [],
      }));
    };
    fetchAllSlugs();
  }, []);
